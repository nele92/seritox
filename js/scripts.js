	...
	var simulateTyping = function () {
		var texts =
			["you before us.",
				...
				"Reliable and built to perfection.",
				"thinking different!",
				"developers with experience.",
				"leaders by vision just what you need.",
				"associates by commitment for everybody.",
				"support 24/7 you", 
				"will like it!"]

		var $ta = $("#typing-simulator");

		function type(string, $element, finished) {
			function writer(i) {
				if (string.length <= i++) {
					$element.html(string);
					setTimeout(function () {
				...
				var rand = Math.floor(Math.random() * (100)) + delay;
		...
		var clear = function ($element, cleared) {
			if ($element.html().length) {
				$element.html($element.html().substring(0, $element.html().length - 1));
				setTimeout(function () { clear($element, cleared) }, 20)
			} else {
				cleared();
			}
		}

		var typeAll = function (texts, $element, pointer) {
			if (pointer < texts.length) {
				type(texts[pointer], $element, function () {
					typeAll(texts, $element, pointer + 1)
				})
			}
		}
